...
export function DateRangePicker({ dateRange, setDateRange, label }: DateRangePickerProps) {
  const [open, setOpen] = React.useState(false)

  // Handler for date selection that will close the popover when both dates are selected
  const handleSelect = (range: DateRange | undefined) => {
    setDateRange(range)
    
    // Close the popover when both from and to dates are selected
    if (range?.from && range?.to) {
      setTimeout(() => setOpen(false), 300) // Small delay for better UX
    }
  }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent 
        className="w-auto p-0 bg-white/10 border border-white/20 backdrop-blur-md rounded-xl" 
        align="start"
      >
        <Calendar
          mode="range"
